...
// JS에서 HTML 태그를 직접 가지고 올 수 있다. AJAX를 쓰지 않고도
function profile_delete() {
    const _realName = document.getElementById("realName").innerText;
    console.log(_realName);
    
    let formData = new FormData();
        formData.append("real_name_give", _realName);
    
    fetch("/profile/delete", { method: "POST", body: formData })
        .then((res) => res.json())
        .then((data) => {
            alert(data["msg"]);
